#!/usr/bin/env python3
"""
ABN BulkExtract → JSONL → Supabase pipeline
Streams all 20 XML files from both ZIPs → filters → writes JSONL → imports to Supabase
...
CAPITAL_POSTCODES = {
    ...
    'Darwin':    list(range(0800, 1000)),  # NT
}
...
def should_keep(rec):
    ...
    if not rec.get('address_state'):
        return False
    pc = rec.get('address_postcode', '')
    try:
        pc_int = int(pc)
        return pc_int in ALL_CAPITAL_POSTCODES
    except (ValueError, TypeError):
        return False

# === PROCESS ALL ZIP PARTS ===
def process_all():
    total_written = 0
    for zip_path in ZIP_PATHS:
        ...
        for xml_name in xml_names:
            part_id = xml_name.split('.')[0].split('_')[-1]  # 01-20
            out_path = OUT_DIR / f"leads_part{part_id}.jsonl"
            print(f"Processing {Path(zip_path).name} :: {xml_name}...")
            fh = z.open(xml_name)
