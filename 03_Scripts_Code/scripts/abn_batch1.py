...
# Files to process (Public02-Public06)
z = zipfile.ZipFile(ZIP_PATH)
xml_files = sorted([n for n in z.namelist() if n.endswith('.xml')])
# Public01 already done; start from index 1 (Public02) to 5 (Public06)
targets = xml_files[1:6]
print(f"Processing {len(targets)} files: {[t.split('/')[-1] for t in targets]}")
...
for xml_name in targets:
    ...
    print(f"  part{part_id}: {kept:,} records")
    total += kept
    context = None
    fh.close()
print(f"\n✅ Batch total: {total:,}")
